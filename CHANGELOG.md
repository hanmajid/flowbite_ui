# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]
### Added
- Add `onChanged` property to `FlowbiteInputField` component.
- Add `FlowbiteModal` component.
- Add `textInputAction`, `autofillHints`, `textCapitalization`, `keyboardType`, and `onSubmitted` properties to `FlowbiteInputField` component.
- Add `FlowbiteToggleSwitch` compponent.
- Add `FlowbiteToggleInput` compponent.

### Changed
- Make `width` nullable in `FlowbiteToast`.
- Rename `FlowbiteToggleInput` to `FlowbiteToggleInputMenu`.

### Fixed
- Fix `FlowbiteBottomNavigation`'s layout for `segmentControls` variant.

## [0.0.8] - 19-09-2026
### Added
- Add all type variants for `FlowbiteStepper` component.
- Add `FlowbiteSidebar` component.
- Add `FlowbiteChatBubble` component.

## [0.0.7] - 15-09-2026
### Added
- Add `FlowbiteSlider` and `FlowbiteRangeSlider` components.
- Add `FlowbiteKbd` component.
- Add `FlowbiteBreakpoint` enum.
- Add `FlowbiteStepper` and `FlowbiteStepperNavLink` components.

### Changed
- Rename `FlowbiteTheme.extension` static method to `FlowbiteTheme.of`.

## [0.0.6] - 12-09-2026
### Added
- Add `FlowbiteBreadcrumb` component.
- Add `FlowbiteToast` component.
- Add `FlowbiteProgressBar` component.
- Add `FlowbiteTooltip` component.

## [0.0.5] - 11-09-2026
### Added
- Add `FlowbiteTheme.extension` static method.
- Add `FlowbiteBottomNavigation` component.
- Add `FlowbiteToggleInput` component (Advanced type).
- Add `FlowbiteJumbotron` component.

### Changed
- Migrate to `material_ui`.

## [0.0.4] - 10-09-2026
### Added
- Add `FlowbiteButtonGroup` component.
- Add `FlowbiteAccordion` component.

### Changed
- Refactor code with dot shorthands.

## [0.0.3] - 09-09-2026
### Added
- Add `FlowbiteAvatar` component.
- Add `FlowbiteAvatarGroupLabel` component.
- Add `FlowbiteAvatarGroup` component.
- Add `FlowbiteBadge` component.

## [0.0.2] - 09-09-2026
### Added
- Add `FlowbiteAlert` component.
- Add `FlowbiteSpinner` component.
- Add Widget Previews for existing components.

### Changed
- Update `README.md`.

## [0.0.1] - 08-09-2026
### Added
- Add `FlowbiteColorsExtension` and `FlowbiteTheme` classes.
- Add `FlowbiteColor` and `FlowbiteColors` classes.
- Add `FlowbiteFontFamily`, `FlowbiteFontSize`, and `FlowbiteFontWeight` classes.
- Add `FlowbiteButton` component.
- Add `FlowbiteOutlinedButton` component.
- Add `FlowbiteIconShape` component.
- Add `FlowbiteInputField` component.
- Add `FlowbiteInputLabel` component.

[Unreleased]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.8...dev
[0.0.8]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.7...v0.0.8
[0.0.7]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.6...v0.0.7
[0.0.6]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.5...v0.0.6
[0.0.5]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.4...v0.0.5
[0.0.4]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.3...v0.0.4
[0.0.3]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.2...v0.0.3
[0.0.2]: https://github.com/hanmajid/flowbite_ui/compare/v0.0.1...v0.0.2
[0.0.1]: https://github.com/hanmajid/flowbite_ui/releases/tag/v0.0.1